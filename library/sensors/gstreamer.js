...
class GStreamer extends tsee_1.EventEmitter {
    constructor(verbose, spawnHelperOverride) {
        super();
        this._handledFiles = {};
        this._lastHash = '';
        ...
        this._spawnHelper = spawnHelperOverride || spawn_helper_1.spawnHelper;
    }
    async init() {
        try {
    ...
    async start(options) {
        ...
        this._tempDir = await fs_1.default.promises.mkdtemp(path_1.default.join(osTmpDir, 'edge-impulse-cli'));
        ...
        if (device.name === 'RZG2L_CRU') {
            ...
            await spawn_helper_1.spawnHelper('media-ctl', ['-d', '/dev/media0', '-r']);
            await spawn_helper_1.spawnHelper('media-ctl', ['-d', '/dev/media0', '-l', "'rzg2l_csi2 10830400.csi2':1 -> 'CRU output':0 [1]"]);
            await spawn_helper_1.spawnHelper('media-ctl', ['-d', '/dev/media0', '-V', "'rzg2l_csi2 10830400.csi2':1 [fmt:UYVY8_2X8/640x480 field:none]"]);
            await spawn_helper_1.spawnHelper('media-ctl', ['-d', '/dev/media0', '-V', "'ov5645 0-003c':0 [fmt:UYVY8_2X8/640x480 field:none]"]);
            if (this._verbose) {
    ...
    async getAllDevices() {
        ...
        try {
            ...
            if (typeof message === 'string' && message.indexOf('Failed to start device monitor') > -1) {
                ...
                lines = (await this._spawnHelper('gst-device-monitor-1.0', ['Video/Source', 'Source/Video', 'Video/CameraSource']))
                    .split('\n').filter(x => !!x).map(x => x.trim());
            }
            else {
                throw ex;
    ...
    async listNvarguscamerasrcDevices() {
        ...
        if (this._spawnHelper === spawn_helper_1.spawnHelper) {
            gstLaunchRet = await new Promise((resolve, reject) => {
